...
2. ~~**`"personality": null`**~~ — ✅ **DECIDED, and not an open question. Do not re-raise.** bhava is **coming**, merely not ported to Cyrius yet, so `personality` stays unported and the wire **keeps emitting `null`** — the default Rust build's bytes. **DEFER, never DROP.** Revisit only when bhava lands in Cyrius.
