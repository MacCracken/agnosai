...
## Deleting `rust-old/` — the release after 2.1.0
...
| B7 | **Tracing export** | Large | The Rust tree exported every `#[tracing::instrument]` span over OTLP (20 of them: sandbox, routes, crew runner, orchestrator, scoring, cost planning); agnosai exports only the three GenAI span kinds. JSON log lines lack span context and per-module targets, which needs sakshi's log hook to expose them. |
