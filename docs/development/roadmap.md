...
| B3 | **Thread the bayan `_a` constructors** — 🟡 **in progress: 1 of 49 modules** | Large (small bites) | **Unblocked 2026-07-31 by cyrius 6.5.5**, which folds bayan 1.4.0. The narrowed upstream ask this row used to carry (`parse_buf_a` / `obj_set_a` / `build_a`) was implemented and released in bayan 1.4.0, so **nothing here waits on anything** — the whole JSON path is arena-able today. ✅ **Bite 1 done: `src/core/json.cyr` + `src/core/task.cyr`** — `agnosai_task_to_json` measured **1792 → 0 bytes/response** on an arena with `reset_via`, wire byte-identical, mutation-verified twice. Convention: `_a` body + bare-name wrapper delegating through `default_alloc()`, matching `str_from_a`/`alloc_via`. **Finding worth carrying into the next bite:** after the JSON cells moved, the entire residual was `map_keys` — it materialises a key vec via `vec_new()` on the global bump and has no `_a` form. `_agnosai_map_to_value_a` now walks the documented slot layout (`lib/hashmap.cyr:22-35`) instead, guarded on `key_type == 2`. Expect the same in any module that serialises a map. **Remaining: 48 `*_to_value` fns, ~645 constructor sites.** Take them a module at a time and measure each — the previous estimate that the `to_wire` spellings were a negligible 2.5% was true only while the other 96% was unfixable; they are now the whole remainder if skipped. |
