...
**63 suites, 4,218 assertions, all passing.** Coverage `cyrius coverage --min 80`
→ **100% (899/899 fns)**, 64/64 files referenced.

```sh
cyrius tests tests        # 63 suites; each prints "N passed" with an "(N total)" suffix
...
plus a final `63 passed, 0 failed` line that counts **suites, not assertions** —
