...
**64 suites, 4,306 assertions, all passing.** Coverage `cyrius coverage --min 80`
→ **100% (899/899 fns)**, 64/64 files referenced.

```sh
cyrius tests tests        # 64 suites; each prints "N passed" with an "(N total)" suffix
...
plus a final `64 passed, 0 failed` line that counts **suites, not assertions** —
