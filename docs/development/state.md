...
| `orchestrator` ported (M5, Phase 4) | 🟡 4 pure leaves + scoring + scheduler + budget + approval done; 7 modules left |
...
**1944 assertions across 32 `.tcyr` suites, all passing** (plus the 2-assertion scaffold smoke):
...
`cyrius coverage --min 80` → **100% (610/610 fns), gate OK**. Shared assertion helpers live in
