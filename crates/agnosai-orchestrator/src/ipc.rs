...
impl IpcServer {
    ...
    pub async fn bind(path: &std::path::Path) -> agnosai_core::Result<Self> {
...
impl IpcConnection {
    ...
    pub async fn send(&mut self, payload: &serde_json::Value) -> agnosai_core::Result<()> {
        let data = serde_json::to_vec(payload)?;
        write_frame(&mut self.stream, &data).await
    }

    /// Receive a JSON payload from the connection.
    pub async fn recv(&mut self) -> agnosai_core::Result<serde_json::Value> {
...
async fn write_frame(stream: &mut UnixStream, data: &[u8]) -> agnosai_core::Result<()> {
    let len = u32::try_from(data.len()).map_err(|_| {
        agnosai_core::AgnosaiError::Ipc(format!("frame too large: {} bytes", data.len()))
    })?;
